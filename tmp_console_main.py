...
class HBNBCommand(cmd.Cmd):
    ...
    def default(self, arg):
        """
        Handles commands in the format <ClassName>.<command>(<args>).
        ...
        cmd_args = rest.split('(', 1)[1].rstrip(')')
        if cmd_name not in commands:
            print("*** Unknown syntax: {}".format(arg))
            return
        if cmd_args:
            # Strip surrounding quotes from each argument
            args_list = [a.strip().strip('"').strip("'")
                         for a in cmd_args.split(',')]
            call_arg = cls_name + " " + " ".join(args_list)
        else:
            call_arg = cls_name
        commands[cmd_name](call_arg)
    ...
    def do_count(self, arg):
        ...
        if arg in self.__class__.classes:
            all_objs = storage.all()
            count = sum(1 for k in all_objs if k.startswith(arg + "."))
            print(count)
        else:
            print("** class doesn't exist **")
